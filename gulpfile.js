...
// var sass = require('gulp-sass-ruby');
...
gulp.task('moveCss',['clean-styles'], function(){
...
gulp.task('sass', function () {
  return gulp.src('./app/styles/**/*.scss')
    .pipe(
        sass({includePaths: 'app/bower_components'})
...
var bundler = watchify(
  ...
  .require('./config/dev.js', {expose: 'config'})
);
...
gulp.task('buildScripts', function() {
    return browserify(sourceFile)
        .require('./config/prod.js', {expose: 'config'})
        .bundle()
        .pipe(source(destFileName))
...
gulp.task('moveLibraries',['clean-styles', 'clean-scripts'], function(){
  // the base option sets the relative root for the set of files,
  // preserving the folder structure
...
// Default task
gulp.task('default', ['clean-styles', 'clean-scripts', 'build'  ]);
